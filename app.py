...
# ==========================================
    ...
    if not st.session_state.admin_logged:
        ...
        if menu_admin == "📊 Fechamento & Financeiro":
            ...
            produtos_opt = carregar_coluna("produtos", "nome") or ["CEBOLA CAIXA 1", "AMEIXA IMPORTADA"]
            fornecedores_opt = carregar_coluna("fornecedores", "fornecedor") or ["BAHIA"]
            grupos_opt = carregar_coluna("grupos", "grupo") or ["GERAL"]
            
            with aba_compra:
                with st.form("form_entrada_estoque"):
                    prod_c = st.selectbox("Produto", produtos_opt, key="comp_prod")
                    fornec_c = st.selectbox("Fornecedor", fornecedores_opt, key="comp_forn")
                    grupo_c = st.selectbox("Grupo", grupos_opt, key="comp_grup")
                    qtd_c = st.number_input("Quantidade Comprada", min_value=0.1, step=1.0, value=10.0)
                    custo_c = st.number_input("Valor de Custo Unitário (R$)", min_value=0.0, step=1.0, value=50.0)
            ...
            tab_c, tab_f, tab_g, tab_p = st.tabs(["Clientes", "Fornecedores", "Grupos", "Novo Produto"])
            
            with tab_c:
                with st.form("form_cad_cliente"):
                    nome_c = st.text_input("Nome / Razão Social do Cliente")
                    ...
                    if st.form_submit_button("Salvar Cliente"):
                        if nome_c:
                            if salvar_cliente_completo(nome_c, tel_c, doc_c, end_c, cidade_c):
                                st.success("Cliente cadastrado com sucesso!")
                                st.rerun()
                            else:
                                st.error("Erro: Este cliente já existe.")
                        else:
                            st.warning("Informe o nome do cliente.")
                            
                st.markdown("---")
                st.subheader("Clientes Cadastrados")
                df_cli = carregar_dados("SELECT * FROM clientes")
                if not df_cli.empty:
            ...
            with tab_f:
                with st.form("form_cad_forn"):
                    ...
                    if st.form_submit_button("Salvar Fornecedor"):
                        if nome_f:
                            if salvar_simples("fornecedores", "fornecedor", nome_f):
                                st.success("Fornecedor salvo!")
                                st.rerun()
                            else:
                                st.error("Fornecedor já cadastrado.")
                        else:
                            st.warning("Informe o nome.")
                st.markdown("---")
                st.subheader("Fornecedores Cadastrados")
                df_forn = carregar_dados("SELECT * FROM fornecedores")
                if not df_forn.empty:
                    st.dataframe(df_forn, use_container_width=True)

            with tab_g:
                with st.form("form_cad_grup"):
                    nome_g = st.text_input("Nome do Grupo / Categoria")
                    if st.form_submit_button("Salvar Grupo"):
                        if nome_g:
                            if salvar_simples("grupos", "grupo", nome_g):
                                st.success("Grupo salvo!")
                                st.rerun()
                            else:
                                st.error("Grupo já cadastrado.")
                        else:
                            st.warning("Informe o nome.")
                st.markdown("---")
                st.subheader("Grupos Cadastrados")
                df_g = carregar_dados("SELECT * FROM grupos")
                if not df_g.empty:
                    st.dataframe(df_g, use_container_width=True)

            with tab_p:
                with st.form("form_cad_prod_geral"):
                    p_nome = st.text_input("Nome do Produto")
                    p_forn = st.selectbox("Fornecedor Padrão", carregar_coluna("fornecedores", "fornecedor") or ["BAHIA"])
                    p_grup = st.selectbox("Grupo Padrão", carregar_coluna("grupos", "grupo") or ["GERAL"])
                    p_custo = st.number_input("Preço de Custo (R$)", min_value=0.0, step=1.0)
                    p_venda = st.number_input("Preço de Venda (R$)", min_value=0.0, step=1.0)
                    p_estoque = st.number_input("Estoque Inicial", min_value=0.0, step=1.0)
                    
                    if st.form_submit_button("Cadastrar / Atualizar Produto"):
                        if p_nome:
                            salvar_produto_completo(p_nome, p_forn, p_grup, p_custo, p_venda, p_estoque)
