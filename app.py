...
def adequar_banco_e_migrar():
    ...
    colunas_verificar_vendas = ['forma_pagamento', 'valor_recebido', 'tipo', 'codigo', 'data', 'status']
...
def salvar_produto_completo(nome, fornecedor, grupo, preco_custo, preco_venda, estoque_inicial):
    ...
    try:
        cursor.execute("""
            INSERT INTO produtos (nome, produto, fornecedor, grupo, valor_compra, valor_venda, estoque_atual, quantidade) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    if not st.session_state.admin_logged:
        ...
        if menu_admin == "🛒 PDV — Frente de Caixa":
                ...
                if st.button("Finalizar Venda no PDV", type="primary"):
                    if not df_caixa_aberto.empty and len(st.session_state.carrinho_pdv) > 0:
                        ...
                        for item in st.session_state.carrinho_pdv:
                            cursor.execute("""
                                INSERT INTO vendas (cliente, produto, quantidade, valor_total, status, data, tipo)
            ...
            with aba_cad:
                ...
                if st.button("Salvar PEDIDO", type="primary", key="btn_salvar_pedido_unico_definitivo"):
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (str(cliente), str(produto), str(fornecedor), str(grupo), float(quantidade), float(preco_unitario), c_total, c_tipo, 'Pendente', data_atual))
                        conn.commit()
                ...
                try:
                    df_parcial = carregar_dados("SELECT id, cliente, produto, quantidade, valor_venda, valor_total, tipo, status FROM vendas WHERE status IS NULL OR status != 'Finalizado' ORDER BY id DESC LIMIT 20")
                except Exception:
