...
def salvar_pedido_ou_venda(cliente, produto, fornecedor, grupo, quantidade, valor_venda, forma_pagamento, valor_recebido, tipo):
    cursor = conn.cursor()
    cursor.execute("INSERT INTO vendas (cliente, produto, fornecedor, grupo, quantidade, valor_venda, valor_total, forma_pagamento, valor_recebido, tipo, codigo, data) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                   (cliente, produto, fornecedor, grupo, quantidade, valor_venda, quantidade*valor_venda, forma_pagamento, str(valor_recebido), tipo, "VEN" if tipo=="VENDA" else "PED", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()

# -----------------------------------------------------------------------------
# FRENTE DE CAIXA (PDV) - LÓGICA
# -----------------------------------------------------------------------------
def finalizar_pdv(cliente, forma_pagamento):
    cursor = conn.cursor()
    for item in st.session_state.carrinho_pdv:
        salvar_pedido_ou_venda(cliente, item['produto'], "GERAL", "GERAL", item['quantidade'], item['valor_unitario'], forma_pagamento, item['valor_total'], "VENDA")
...
if perfil == "👤 Portal do Cliente":
    st.title("Portal do Cliente - Em desenvolvimento")
else:
    if not st.session_state.admin_logged:
        ...
        if menu == "💸 Frente de Caixa (PDV)":
            ...
            with col_prod: prod_sel = st.selectbox("Produto", carregar_coluna("produtos", "nome"))
            with col_qtd: qtd_sel = st.number_input("Qtd", min_value=0.1, value=1.0)
            with col_add: 
                if st.button("➕ Adicionar"):
                    preco = float(carregar_dados(f"SELECT valor_venda FROM produtos WHERE nome = '{prod_sel}'").iloc[0,0])
            ...
            if st.session_state.carrinho_pdv:
                ...
                if st.button("✅ FINALIZAR VENDA"):
                    finalizar_pdv(cliente_pdv, forma_pdv)
